<x-layouts.app :title="__('Build Course Structure')">
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 p-4" x-data="{ isAddingCourses: false, isExportVisible: false, selectedIntakesForExport: [] }">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-lg shadow p-4 mb-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400">View and manage course structures by programme and intake
                    </p>
                </div>
                <div class="flex gap-2">
                    ...
                    <button @click="isAddingCourses = true" x-show="!isAddingCourses"
        ...
        <!-- Export Form -->
        <div x-show="isExportVisible" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="space-y-4">
                @foreach ($intakes as $intake)
                    <div class="flex items-center">
                        <input type="checkbox" x-model="selectedIntakesForExport" value="{{ $intake }}"
                            id="export-intake-{{ $intake }}"
                            class="rounded border-gray-300 dark:border-zinc-700 text-indigo-600">
                        <label for="export-intake-{{ $intake }}"
            ...
            <div class="mt-4 flex justify-end gap-3">
                <button type="button" @click="isExportVisible = false"
        ...
        <div x-show="isAddingCourses" x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0 transform -translate-y-2"
            x-transition:enter-end="opacity-100 transform translate-y-0"
            x-transition:leave="transition ease-in duration-150"
            x-transition:leave-start="opacity-100 transform translate-y-0"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        <label for="programme_id"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Programme</label>
                        <select id="programme_id" name="programme_id" required
                            ...
                            @foreach ($programmes as $programme)
                                <option value="{{ $programme->id }}">{{ $programme->code }} - {{ $programme->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="intake"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Intake</label>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-zinc-700" style="display: block;">
                        <thead class="bg-gray-50 dark:bg-zinc-800"
                            style="display: table; width: calc(100% - 1rem); table-layout: fixed;">
                            <tr>
                                <th style="width: 10%"
        ...
        <div x-data="{ selectedIntake: null }" class="space-y-4">
            ...
            <div x-show="selectedIntake !== null" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 transform -translate-y-2"
                x-transition:enter-end="opacity-100 transform translate-y-0"
                x-transition:leave="transition ease-in duration-150"
                x-transition:leave-start="opacity-100 transform translate-y-0"
                ...
                <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-zinc-700">
                    <thead class="bg-gray-50 dark:bg-zinc-800">
                        <tr>
                            <th
                                ...
                                Course Code</th>
                            <th
                                class="w-[20%] px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                Course Name</th>
                            <th
                                class="w-[8%] px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                Credit Hours</th>
                            <th
                                class="w-[10%] px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                Section</th>
                            <th
                                class="w-[12%] px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                Faculty</th>
                    ...
                    <tbody class="bg-white dark:bg-zinc-900 divide-y divide-gray-200 dark:divide-zinc-700">
                        @forelse($rules as $rule)
                            <tr x-show="selectedIntake === '{{ $rule->intake }}'"
                            ...
                            <tbody class="bg-white dark:bg-zinc-900 divide-y divide-gray-200 dark:divide-zinc-700">
                                @foreach ($allSections as $section)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                                            {{ $rules->where('intake', $intake)->where('course.section', $section)->sum('course.credit_hour') }}
                                        </td>
                                    </tr>
                                @endforeach
                                <!-- Total Row -->
    ...
    <script>
        function deleteRule(ruleId) {
            const formId = 'deleteRuleForm';
            const form = document.getElementById(formId);
            form.action = `{{ route('admin.buildcoursestruct.destroy', ['id' => '__ID__']) }}`.replace('__ID__', ruleId);
