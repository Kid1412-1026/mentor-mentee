<x-layouts.app :title="__('Manage Announcements')">
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="overflow-x-auto bg-white dark:bg-zinc-900 shadow-md dark:shadow-zinc-800/30 rounded-lg">
            ...
            <div class="p-6" x-data="{ isAddingAnnouncement: false }">
                <!-- Toggle Button -->
                ...
                <div x-show="isAddingAnnouncement" x-transition:enter="transition ease-out duration-200"
    ...
    <script>
        ...
        function editAnnouncement(id) {
            ...
            const url = `{{ route('admin.news.edit', ['id' => '__ID__']) }}`.replace('__ID__', id);
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    document.getElementById('editAnnouncementForm').action =
                        `{{ route('admin.news.update', ['id' => '__ID__']) }}`.replace('__ID__', id);
                    document.getElementById('editAnnouncementModal').classList.remove('hidden');
                })
                .catch(error => {
                    console.error('Error:', error);
                    alert('Error fetching announcement data: ' + error.message);
                });
        }

        function deleteAnnouncement(id) {
            const formId = 'deleteAnnouncementForm';
            const form = document.getElementById(formId);
            form.action = `{{ route('admin.news.destroy', ['id' => '__ID__']) }}`.replace('__ID__', id);
