<x-layouts.app :title="__('Manage Career')">
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="overflow-x-auto bg-white dark:bg-zinc-900 shadow-md dark:shadow-zinc-800/30 rounded-lg">
            <!-- Add Career Section -->
            <div class="p-6" x-data="{ isAddingCareer: false }">
                <!-- Toggle Button -->
                ...
                <div x-show="isAddingCareer" x-transition:enter="transition ease-out duration-200"
                    ...
                    <form action="{{ route('admin.career.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Attachment</label>
            ...
            <div class="mt-3">
                ...
                <form id="editCareerForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-end gap-3">
                            <button type="button"
                                onclick="document.getElementById('editCareerModal').classList.add('hidden')"
                                class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors">
                                Cancel
                            </button>
                            <button type="submit"
    ...
    <script>
        function editCareer(id) {
            ...
            const url = `{{ route('admin.career.edit', ['id' => '__ID__']) }}`.replace('__ID__', id);
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    document.getElementById('editCareerForm').action =
                        `{{ route('admin.career.update', ['id' => '__ID__']) }}`.replace('__ID__', id);
                    document.getElementById('editCareerModal').classList.remove('hidden');
                })
                .catch(error => {
                    console.error('Error:', error);
                    alert('Error fetching career data: ' + error.message);
                });
        }

        function deleteCareer(id) {
            const formId = 'deleteCareerForm';
            const form = document.getElementById(formId);
            form.action = `{{ route('admin.career.destroy', ['id' => '__ID__']) }}`.replace('__ID__', id);
